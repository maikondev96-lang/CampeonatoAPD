      ...
      <div className="fs-comps-section" style={{ marginTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
        ...
        <div className="fs-comps-list">
          {competitions.length === 0 && !loading && (
            <div style={{ padding: '2rem', textAlign: 'center', opacity: 0.5 }}>Nenhum campeonato ativo encontrado.</div>
          )}
              ...
              <div key={comp.id} className="fs-comp-item-admin">
                <div className="fs-comp-item-main">
                  ...
                  <div className="fs-comp-details">
                    <span className="fs-comp-country" style={{ fontSize: '0.65rem', fontWeight: 800, color: comp.is_active ? 'var(--primary-color)' : 'var(--text-muted)' }}>
                      {comp.is_active ? '● ATIVO' : 'ARQUIVADO'}
                    </span>
                    <h3 className="fs-comp-name" style={{ fontSize: '1rem', marginTop: '2px' }}>{comp.name}</h3>
                    <span className="fs-comp-type">{comp.type.toUpperCase()}</span>
                  </div>
                </div>
                <div className="fs-comp-actions">
                  <Link to={`/admin/${comp.slug}/2026`} className="btn-app-outline">Gerenciar</Link>
                  <Link to={`/admin/${comp.slug}/2026/settings`} className="btn-app-icon"><Settings size={16}/></Link>
                </div>
              </div>
            );
          })}
